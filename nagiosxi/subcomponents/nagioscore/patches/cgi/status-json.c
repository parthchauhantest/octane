...
int main(void){
	...
	for(temp_host=host_list;temp_host!=NULL;temp_host=temp_host->next){
		...
		if(temp_hoststatus->accept_passive_host_checks==1)
			str="true";
		else
			str="false";
		printf("\t\t\t\t\"passiveChecks\" : %s,\n",str);

		if(temp_hoststatus->obsess_over_host==1)
			str="true";
		else
			str="false";
		printf("\t\t\t\t\"obsessing\" : %s,\n",str);
		...
		for(temp_service=service_list;temp_service!=NULL;temp_service=temp_service->next){
			...
			if(temp_servicestatus->accept_passive_service_checks==1)
				str="true";
			else
				str="false";
			printf("\t\t\t\t\t\t\"passiveChecks\" : %s,\n",str);

			if(temp_servicestatus->obsess_over_service==1)
				str="true";
			else
				str="false";
			printf("\t\t\t\t\t\t\"obsessing\" : %s,\n",str);
