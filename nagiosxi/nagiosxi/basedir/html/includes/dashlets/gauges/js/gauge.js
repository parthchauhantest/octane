function create_gauge(container, modifier, configin) {
    ...
    var config = {  'size': $('#'+container).width(),
                    'min': 0,
                    'max': 100,
                    'fontsize': 12,
                    'minorTicks': 5,
    ...
    for (var attrname in configin) { config[attrname] = configin[attrname]; }
...
if(gaugejson==''){
                url=base_url+"includes/dashlets/gauges/getdata.php";
                $.ajax({"url": url,
                            async: false,
...
servicelist = "<option selected='selected'/>"
var host = $("#gauges_form_name").val()
for(k in gaugejson[host]){
	servicelist += "<option value='"+k+"'>"+k+"</option>";
}
...
function getgaugeservices(){
    ...
    dslist = "<option selected='selected'/>"
    var host = $("#gauges_form_name").val()
    var service=$("#gauges_form_services").val()
    for(k in gaugejson[host][service]){
        dslist += "<option value='"+k+"'>"+k+"</option>";
    }
