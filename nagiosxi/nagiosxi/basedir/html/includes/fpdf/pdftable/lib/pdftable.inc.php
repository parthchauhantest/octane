 ...
 * FROM: http://www.vanxuan.net/tool/pdftable/
 */
//require_once('fpdf.inc.php');
require_once(dirname(__FILE__).'/../../fpdf_multicelltag/MultiCellTag/class.multicelltag.php');
...
class PDFTable extends fpdf_multicelltag{
	...
	foreach ($t->name as $i=>$element){
		...
		switch ($element){
			case 'table':
				...
				if (isset($a['align']))		$table['a']	= strtolower($a['align']);
