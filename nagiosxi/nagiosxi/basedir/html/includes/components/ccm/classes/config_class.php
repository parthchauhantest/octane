...
class nagconfig {
  ...
  function nagconfig() {
    ...
    //print $this->arrSettings['path']['tempdir'];
    ...
    if (($booReturn == true) && isset($arrDataset['last_modified'])) 
    {
      ...
      if (($strMethod == 1) && (file_exists($strBaseDir."/".$strFile))) 
      {
        ...
        if (strtotime($strTimeTable) > $intFileStamp) 
				$strCheckConfig = gettext("<span class='urgent'><strong>Apply Configuration</strong> for new changes to take effect.</span>");
        		//$strCheckConfig = gettext("<span class='urgent'>Notice: <strong>{$strFile}</strong> file is out of date!</span>");
        return(0);
      } 
    }
    return(1);
  ...
  function moveFile($strType,$strName) {
    ...
    if (file_exists($strConfigDir."/".$strName) && is_writable($strBackupDir) && is_writable($strConfigDir)) {
        ...
        //added sanity check to make sure the file actually got deleted!!!! - MG 
        //echo  "UNLINKING: ".$strConfigDir."/".$strName."<br />";
        if(!unlink($strConfigDir."/".$strName)) {
        	$this->strDBMessage = gettext('Config file: '.$strConfigDir."/".$strName.' failed to delete, check permissions and delete file manually!');
        	return (1);
        }	
    } 
        ...
        else //file is already removed, shouldn't matter from here 
        	return 0;  		    
    }
    if(!is_writable($strBackupDir)) {
  ...
  if (file_exists($strName)) {
    if(!unlink($strName)) { //added check to see if the file is actually deleted 
  ...
  function createConfig($strTableName,$intMode=0) {
    ...
    $strVersion = "Nagios 3.x config file";
...
//          }
    ...
    if ($intMode == 0) {
      ...
      $this->strDBMessage = gettext("<span class='successMessage'>".gettext('Configuration file').": <strong>$strFile</strong> ".gettext('successfully written!')."</span>");
      return(0);
    } else if ($intMode == 1) {
      $configtp->show();
      return(0);
